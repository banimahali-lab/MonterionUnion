...
export default function UserDashboard({ user, goToHome, onLogout, onCreditCardReject }) {
    ...
    const [transferTab, setTransferTab] = useState('monterion');
    ...
    if (data.error) {
        return (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '100vh', backgroundColor: 'var(--black-bg)', textAlign: 'center', padding: '20px' }}>
                <i className="fas fa-exclamation-triangle" style={{ fontSize: '3rem', color: '#ff5252', marginBottom: '15px' }}></i>
                <h2 style={{ color: 'var(--text-main)' }}>Mbiri Sanapezeke</h2>
                <p style={{ color: 'var(--text-muted)', maxWidth: '400px', margin: '10px 0 20px 0' }}>Talephera kutsegula akaunti yanu. Mwina inalembetsedwa mosakwanira kapena pali vuto pa intaneti.</p>
                <button onClick={onLogout} style={{ padding: '12px 24px', background: 'var(--light-blue)', color: 'white', borderRadius: '8px', border: 'none', cursor: 'pointer', fontWeight: 'bold' }}>Tulukani</button>
            </div>
        );
    }
        ...
        <div className="dashboard-layout">
            ...
            <header className="mobile-header mobile-only">
                <div className="mobile-user">
                    <div className="avatar-circle-sm">{firstName[0]}</div>
                ...
                <button className="icon-btn" onClick={onLogout}>
                    <i className="fas fa-arrow-right-from-bracket logout-icon"></i>
                </button>
            </header>

            <main className="dash-content">
                {/* OPTIMIZED BALANCE BANNER */}
                <div className="balance-banner mobile-only">
                    <p>Ndalama Zonse</p>
                ...
                <div className="cards-scroll-container">
                    <div className="bank-card gold-card animate-slide-up"
                        ...
                        <div className="card-balance">
                            <small className="desktop-only" style={{color: 'var(--text-muted)'}}>Ndalama Zomwe Zilipo</small>
                            <h3>MWK {bankBalance.toFixed(2)}</h3>
                        </div>
                        <div className="card-bottom">
                            <span className="acc-num">{displayAccNum}</span>
                ...
                <div className="quick-actions-compact animate-slide-up delay-2">
                    <button className="action-btn-sm" onClick={() => setShowTransferModal(true)}>
                        <div className="icon-box-sm"><i className="fas fa-paper-plane"></i></div>
                        <span>Kutumiza</span>
                    </button>
                    <button className="action-btn-sm" onClick={handleMaintenance}>
                        <div className="icon-box-sm"><i className="fas fa-file-invoice"></i></div>
                        <span>Mabilu</span>
                    </button>
                    <button className="action-btn-sm" onClick={handleStatementClick}>
                        <div className="icon-box-sm"><i className="fas fa-chart-pie"></i></div>
                        <span>Sititimenti</span>
                    </button>
                    <button className="action-btn-sm" onClick={handleApplyCard}>
                        <div className="icon-box-sm"><i className="fas fa-plus"></i></div>
                        <span>Khadi</span>
                    </button>
                </div>

                {/* Desktop Quick Actions (Hidden on Mobile) */}
                <div className="quick-actions desktop-only animate-slide-up delay-2">
                ...
                <div className="transactions-section animate-slide-up delay-3">
                    <div className="section-header" onClick={() => setShowHistoryModal(true)} style={{display:'flex', justifyContent:'space-between', cursor:'pointer', marginBottom: '20px'}}>
                        <h3 style={{margin:0, color:'var(--text-main)'}}>Mbiri Ya Zochitika</h3>
                        <span style={{color:'var(--light-blue)', fontSize:'0.85rem', fontWeight:'600'}}>Onani Zonse <i className="fas fa-chevron-right"></i></span>
                    </div>
            ...
            {showCardModal && (
                <div className="fullscreen-modal animate-slide-up-full">
                    ...
                    <div className="modal-body card-manage-body">
                        <div className="cards-scroll-container" style={{paddingBottom: '20px'}}>
                        ...
                        <div className="manage-options-grid">
                            ...
                            <div className="manage-option-item" onClick={() => {setShowChat(true); resetChat();}}>
                                <div className="manage-icon" style={{color:'var(--neon-blue)'}}><i className="fas fa-headset"></i></div><span>Thandizo</span>
                            </div>
                        </div>
                        ...
                        {showChat && (
                            <div className="chat-overlay animate-slide-up">
                                <div className="chat-header">
                                    <div className="bot-profile"><div className="bot-avatar"><i className="fas fa-robot"></i></div><span>Monterion Bot</span></div>
            ...
            {showLimitsModal && (
                <div className="fullscreen-modal animate-slide-up-full">
                    ...
                    <div className="modal-body">
                        ...
                        <button className="btn-cta full-width" style={{marginTop: '20px'}} onClick={() => {setShowLimitsModal(false); showToast("Malire asinthidwa", 'info');}}>Sungani</button>
                    </div>
                </div>
            )}
                ...
                <div className="fullscreen-modal animate-slide-up-full">
                    ...
                    <div className="modal-body" style={{display:'flex', flexDirection:'column', gap:'15px', justifyContent:'center'}}>
                        <div style={{textAlign:'center', marginBottom:'20px', color:'#999'}}>
                            <i className="fas fa-shield-halved" style={{fontSize:'3rem', color:'var(--light-blue)', marginBottom:'15px'}}></i>
                            <p>Sankhani mtundu wa kutseka pa khadi lanu lomaliza ndi 3027.</p>
                        </div>
                        <button className="btn-login-outline full-width" onClick={handleBlockSubmit}>Kutseka Kwakanthawi</button>
                        <button style={{background: '#d32f2f', color: '#fff', padding: '15px', border: 'none', borderRadius: '10px', fontWeight: 'bold'}} onClick={handleBlockSubmit}>Kutseka Kwathunthu (Kuba/Kutaya)</button>
                    </div>
                </div>
            )}
                        ...
                        <div className="transfer-form">
                            {transferTab === 'other' && (
                                <div className="input-group">
                                    <label style={{color: 'var(--text-muted)'}}>Sankhani Banki</label>
                            ...
                            <div className="input-group"><label style={{color: 'var(--text-muted)'}}>Nambala ya Akaunti</label><input type="number" className="dash-input" placeholder="0000 0000 0000" /></div>
                            <div className="input-group"><label style={{color: 'var(--text-muted)'}}>Tsimikizani Akaunti</label><input type="number" className="dash-input" placeholder="Bwerezani nambala" /></div>
                            <div className="input-group"><label style={{color: 'var(--text-muted)'}}>Dzina la Woulandira</label><input type="text" className="dash-input" placeholder="Dzina lonse" /></div>
                            <div className="input-group"><label style={{color: 'var(--text-muted)'}}>Kuchuluka kwa Ndalama (MWK)</label><input type="number" className="dash-input" placeholder="0.00" /></div>
            ...
            {/* 6. DATE PICKER */}
            {showDateSelect && (
                <div className="modal-overlay" onClick={() => setShowDateSelect(false)} style={{background: 'rgba(0,0,0,0.8)'}}>
                    <div className="date-picker-box" onClick={e => e.stopPropagation()} style={{background: 'var(--dark-surface)', padding: '30px', borderRadius: '20px'}}>
                        <h3 style={{color: 'var(--text-main)'}}>Sankhani Nthawi</h3>
                        <input type="date" className="dash-input" onChange={onDateSelected} />
                        <button className="btn-login-outline full-width" onClick={() => setShowDateSelect(false)}>Letsani</button>
                    </div>
                </div>
            )}
